...
public class MemberController {
	...
	public String login(HttpServletRequest req) {
		...
		// * forward  방법 : prefix/suffix를 제외한 나머지 jsp경로 작성 
...
//	@RequestMapping(value="/member/login", method = RequestMethod.POST)
	...
	public String logout(SessionStatus status) {
		...
		//       session.invalidate();
		//  -> 로그아웃 안됨
		// why ? @SessionAttributes로 session scope에 등록된 값을 무효화시키려면 
		// SessionStatus라는 별도의 객체를 이용해야한다.
	...
	public String signUp(Member inputMember /* 커맨드 객체 */,
		...
		// 한글이 깨지는 이유
		// -> POST 요청시 인코딩 처리 필요 -> 인코딩 필터 처리(web.xml)
